...
export class AnalyticsController {
  ...
  @ApiResponse({ status: 404, description: 'No Stellar public key linked to account' })
  async getAllocation(@CurrentUser() user: { id: string; publicKey?: string }): Promise<AssetAllocationDto> {
    if (!user.publicKey) {
      throw new NotFoundException('No Stellar public key linked to this account');
    }
